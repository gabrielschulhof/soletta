...
var systemClockChangeCount = 0;

function systemClockChangeHandler( theDate ) {
	console.log( "system clock has changed. The new date is: " + theDate );
	systemClockChangeCount++;
	if ( systemClockChangeCount >= 2 ) {
		setTimeout( function() {
			console.log( "enough system clock changes. Removing monitor" );
			soletta.sol_platform_del_timezone_monitor( systemClockChangeHandler );
			console.log( "enough system clock changes. Monitor removed" );
			systemClockChangeCount = 0;
		}, 1000 );
	}
}

soletta.sol_platform_add_system_clock_monitor( systemClockChangeHandler );
/*
